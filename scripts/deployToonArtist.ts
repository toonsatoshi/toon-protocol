...
export async function run(provider: NetworkProvider, args: string[]) {
    ...
    const jettonMasterAddressStr = process.env.TOON_JETTON_ADDRESS;

    if (!registryAddressStr || !jettonMasterAddressStr) {
        throw new Error('TOON_REGISTRY_ADDRESS and TOON_JETTON_ADDRESS not set in .env');
    }

    const registryAddress = Address.parse(registryAddressStr.replace(/"/g, '').trim());
    const jettonMasterAddress = Address.parse(jettonMasterAddressStr.replace(/"/g, '').trim());
